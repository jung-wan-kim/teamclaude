...
test('persistent upstream 429 terminates with a bounded number of retries', async () => {
  ...
  try {
    ...
    await res.text();
    assert.equal(res.status, 429);                       // returns 429 instead of hanging
    assert.ok(upstreamHits >= 1 && upstreamHits <= 4,    // a few bounded retries, not infinite
      `expected bounded retries, got ${upstreamHits}`);
    assert.equal(am.accounts[0].status, 'throttled');    // account throttled, not retried forever
  } finally {
    proxy.close();
    upstream.close();
  }
