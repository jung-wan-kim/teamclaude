...
| `tokenRefreshIntervalMs` | How often (ms) the token keep-alive sweep runs (optional, default `300000` = 5 min; `0` = disable). Each sweep sequentially refreshes OAuth tokens that are expiring/expired — including parked (`error`) and disabled accounts, so their refresh-token chains stay alive — and a refresh-caused `error` self-heals back into rotation on success (an upstream-auth `error` stays parked until re-import/login) |
| `profileRefreshIntervalMs` | How often (ms) the subscription-profile sweep runs (optional, default `21600000` = 6 h; `0` = disable). Each sweep fetches every OAuth account's profile — subscription status, rate-limit tier, billing anniversary — so the TUI shows a tier badge (`Max 20x`/`Max 5x`/`Pro`) and a `↻M/D` renewal estimate per account, and a subscription turning unhealthy (`past_due`, `canceled`, …) raises an activity-log alert and a persistent red status in the account row. The renewal date is an **estimate** (next monthly anniversary of the subscription's creation date, clamped for short months) — the API exposes no authoritative period end |
