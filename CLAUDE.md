...
- **Explicit priority** (`setPriority`, config `priority: <int>`): `_selectBest` sorts by `_priority` **first** (lower number = preferred), then the existing reset/util use-or-lose as tiebreakers. Unset is `null` → `_priority` returns `Number.MAX_SAFE_INTEGER`, so **when no account sets a priority every account ties there and the sort falls through to the original use-or-lose unchanged** (the hardened behavior and its tests are untouched). CLI `teamclaude priority <name> <n|clear>`.
