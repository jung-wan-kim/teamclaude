...
**Model-scoped weekly windows** (`anthropic-ratelimit-unified-7d_<label>-*`, currently `7d_oi` — the separate weekly limit for the top model tier, shown as "Fable" in Claude's usage UI) are parsed generically into `quota.modelWeekly` and surfaced in the TUI (third `Fbl` bar on wide terminals) and `teamclaude status`. They are **display-only — never fed into `_isNearQuota`/availability**, because an account over its Fable weekly limit still serves every other model; excluding it would wrongly block Sonnet/Haiku traffic. These headers only appear on responses to requests *for that model tier*, so the value updates only when such traffic flows through the account. **Known limitation:** on an actual Fable-quota 429 the *top-level* `unified-status` is `rejected` (the binding claim is reflected there — verified against live traffic), so the existing exhaustion path still throttles that account globally for the clamped `retry-after` (≤5 min) before failing the request over. Cross-model traffic on that account is blocked for those minutes; avoiding that would need per-model routing (skip Fable-exhausted accounts only for Fable requests), a separate feature.
