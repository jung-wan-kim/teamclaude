...
- When every available account is at its cap, `acquireAccount` **queues** the request (FIFO `_waiters`) up to `config.overflowQueueTimeoutMs` (default 15s); a `releaseAccount` drains waiters. Timeout → `null` → the client gets a `429`. If instead *no account is available at all* (quota-exhausted, not merely capped) it returns `null` immediately (no pointless wait). The queue is bounded by `config.overflowQueueMaxDepth` (default 256) and request bodies by `config.maxRequestBytes` (default 32 MiB → `413`); `server.js` also fast-rejects before buffering when the fleet is saturated and the queue is full — so a request flood (localhost auth is skipped) can't grow the backlog or buffered bodies without bound.
