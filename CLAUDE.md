...
- **Connection affinity (prompt-cache locality):** `acquireAccount(..., affinityKey)` takes the client **socket** (`req.socket`) as a soft key. The first acquire on a connection remembers its account in a `WeakMap` (`_affinity`, value is the account *object* + an `accounts[i] === a` identity guard against a `removeAccount` re-index; GC'd when the socket closes); subsequent acquires on the same connection **prefer that account**. It's *soft* — honored only if that account is still available, has a free slot, and isn't `exclude`d, else it falls through to normal `_tryAcquire`. So HTTP/1.1 keep-alive (a session's **sequential** turns reuse one socket) keeps a terminal on one account (cache warm), while a **concurrent** burst — which necessarily opens separate sockets — still spreads by the cap logic. It never exceeds a cap or revives an exhausted account, so the 429 guarantees and use-or-lose for *new* connections are unchanged. Crucially this also blunts the periodic `reevalIntervalMs` re-prioritization: when the global sticky primary moves, live connections keep their own account (no mass cache-bust); only new connections follow the new primary. Disable with `config.sessionAffinity: false` (route purely by use-or-lose every request).
